...
void Chinchetario::init() {
	...
	//^^^^^^^Las pistas actuales las lleva el storyManager, no hace falta crearlas aqu� de nuevo
	//for (int i = 0; i < 6; i++) {
	//	Entity* pista = entityManager_->addEntity(Layers::DragDropLayer);
	//	Transform* pTR = pista->addComponent<Transform>();
...
void Chinchetario::a�adePista() {
	...
	if (dd_ != nullptr) {
		...
		if (!dd_->getDragging()) {
			...
			Vector2D mousePos = ih->getMousePos();			 //Guarda la posici�n del rat�n
			...
			if (SDL_PointInRect(&p, &invRect)) {
				//la vuelve a poner en la posici�n inicial
				scroll_->addItem(pTR);
		...
		//pilla qu� pista est� siendo agarrada
